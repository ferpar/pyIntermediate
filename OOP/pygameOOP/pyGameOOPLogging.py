...
'''
DEBUG       Detailed information, tupically of interest only when diagnosing problems.
INFO        Confirmation that things are working as expected.
WARNING     An indication that something unexpected happened, or indicative of some problem in the network
ERROR       Due to a more serious problem, the software has not been able to perform some function.
CRITICAL    A serious error, indicating that the program itelsf may be unable to continue running.
'''
...
class BlueBlob(Blob):
    ...
    def __add__(self, other_blob):
        ...
        if other_blob.color == (255, 0, 0):
            ...
            raise Exception('Tried to combine one or multiple blobs with unsupported colors.')
        ...
        for other_blobs in blues, reds, greens:
            for other_blob_id, other_blob in other_blobs.copy().items():
                ...
                if blue_blob == other_blob:
                    ...
                    if are_touching(blue_blob, other_blob):
